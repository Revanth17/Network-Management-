...
<?php
	...
	$units = array('Total Kbytes' => '', 'CPU Utilization' => '%%', 'Requests/sec' => 'rps', 'Bytes/sec' => 'Bps', 'Bytes/request' => 'B');
		...
		while($row = mysqli_fetch_assoc($result))
		{
			...
			{	
				foreach($met_list as $key)
				{
					...
					array_push($opts,
								"GPRINT:max_" . $match[$key] . '_' . $unique . ": %3.2lf" . $units[$key] . "\\t",
								"GPRINT:avg_" . $match[$key] . '_' . $unique . ": %3.2lf" . $units[$key] . "\\t",
								"GPRINT:last_" . $match[$key] . '_' . $unique . ": %3.2lf" . $units[$key] . "\\n");
					}
					else{
						array_push($opts, "LINE1:" . $match[$key] . '_' . $unique . "#" . $colour . ":" . $key . "\\t\\t");
