...
$dbh->do("CREATE TABLE IF NOT EXISTS RESULTS21 (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
										   UNIQUE KEY (IP))") or die $DBI::errstr;

#Inserting IP values RESULTS21 into  table from Devices table
#$dbh->do("INSERT INTO RESULTS21 (IP) SELECT SERVERS.IP from SERVERS ON DUPLICATE KEY UPDATE IP=RESULTS21.IP")
#		or die $DBI::errstr;

#Getting servers SERVERS_sai from database
$servers_data = $dbh->selectall_hashref("SELECT * from RESULTS21", 'id');
