...
//Drop RESULTS22 table
$query = "DROP TABLE IF EXISTS RESULTS22";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
//Drop RESULTS23 table
$query = "DROP TABLE IF EXISTS RESULTS23";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));


//Creating RESULTS22 table
$query = "CREATE TABLE RESULTS22 (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
...
$query = "CREATE TABLE RESULTS23 (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
										   metrics LONGTEXT,
										   UNIQUE KEY (IP))";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
			...
			<!--Status panel-->
				<div class = "col-md-12" style = "height: 100%;">
					<div class = "container-fluid" style = "margin: 0 20px 0 20px; padding: 0px; height: 100%;">
						<h3 style = "text-align: center; color: black; padding-bottom: 10px">Servers</h3>
...
echo '	<div class = "row" style = "margin: 0; height: 100%;">
		<h3 style = "text-align: center; color: black; padding-bottom: 10px">Devices</h3>
		<table class = "table table-bordered" style = "width: 100%; text-align: center;">
			<tr style = "text-align: center; padding: 2px;">';
...
//SERVERS
	...
	{
		$query = "INSERT INTO RESULTS23 (IP) VALUES ('" . $insert . "') ON
					DUPLICATE KEY UPDATE id = RESULTS23.id";
		mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	}
...
//DEVICES
	...
	foreach($_GET['if_list'] as $input => $output)
	{
		...
		if($output == "all")
		{
			$query = "SELECT selected_list, selected_name from RESULTS2 WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
			$result = mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	...
	foreach(array_keys($device) as $dev)
	{
		...
		$query = "INSERT INTO RESULTS22 (IP, PORT, COMMUNITY, Interface_List, Interface_Name) 
