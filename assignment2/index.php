...
<!---Servers Table-->
					<div class = "row" style = "margin: 0; height: 100%;">
						<table class = "table table-bordered" style = "width: 100%;">
							<thead>
								<h3 style = "width: 100%; text-align: center; color: black; padding-bottom: 10px">
...
{
	$query = "INSERT INTO RESULTS2 (IP, PORT, COMMUNITY) VALUES ('" . $_GET['ip'] . "', '" . $_GET['port'] . "', '" . $_GET['community'] . "')
...
if(isset($_GET['button']) && !empty($_GET['if_list']))
{
	...
	foreach($_GET['if_list'] as $input => $output)
	{
		...
		if($output == "all")
		{
			$query = "SELECT Interface_List, Interface_Name from RESULTS2 WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
	...
	foreach(array_keys($device) as $dev)
	{
		...
		$query = "UPDATE RESULTS2 SET selected_list = '" . $device[$dev]["iflist"]['string'] . "'" . ', selected_name = ' . "'" . $device[$dev]["ifname"]['string'] . "' 
...
//Table to select INTERFACES of a device to monitor from RESULTS2
...
if(isset($_GET['button']))
{
	if(!empty($_GET['device_monitor']))
	{
		foreach($_GET['device_monitor'] as $insert)
		{
			...
			$query = "DELETE FROM RESULTS2 WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
			mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
		}
...
{
	$query = "INSERT INTO RESULTS21 (IP) VALUES ('" . $_GET['server_ip'] . "') ON DUPLICATE KEY UPDATE id=id";
	mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	echo 'Server added';
}
...
if(isset($_GET['button']))
{
	if(!empty($_GET['server_monitor']))
	{
		...
		{
			$query = "DELETE FROM RESULTS21 WHERE IP = '" . $insert . "'";
			mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
		}
...
//Table to Remove a server from RESULTS21
$query = "SELECT * FROM RESULTS21";
$result = mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
