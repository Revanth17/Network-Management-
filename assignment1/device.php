...
if(isset($_GET['device']))
{
	...
	//Statement handle
	$query = "SELECT * FROM RESULTS1 WHERE IP='".$exploded[0]."' AND PORT='".$exploded[1]."' AND COMMUNITY='".$exploded[2]."'";
	$result = mysqli_query($connection, $query);
	$row = mysqli_fetch_assoc($result);
	echo '<h3 style = "width: 100%; text-align: center; color: black;"> Traffic Analysis for Device: </nbsp>' . $row['sysName'] . '</h3><h3>Interface: </nbsp>' . $exploded[4] . '</h3>';
?>			
			</div>
...
if(file_exists($file))
{
	...
	foreach($types as $i)
	{
		...
		echo '<tr style = "text-align: left; padding: 2px;">
		...
		if($i == '-1m')
		{
			echo '<h4></h4></br>	</nbsp>
					...
					<td>';
			array_push($opts, "--title= Monthly Graph", "--x-grid", "WEEK:1:WEEK:1:WEEK:1:604800:%V");
		}
		...
		if($i == '-1y')
		{
			echo '<h4></h4></br>	</nbsp>
					...
					<td>';
			array_push($opts, "--title= Yearly Graph", "--x-grid", "MONTH:1:MONTH:1:MONTH:1:2419200:%b");
		}
